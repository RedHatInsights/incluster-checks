...
class TestCLI:
    ...
    def test_format_junit_exits_with_deprecation_error(self, capsys):
        """Test that --format junit exits with error."""
    ...
    def test_format_json_exits_with_deprecation_error(self, capsys):
        """Test that --format json exits with error."""
        test_args = ['in-cluster-checks', '--format', 'json']
        with patch.object(sys, 'argv', test_args):
            with pytest.raises(SystemExit) as exc_info:
                main()
            assert exc_info.value.code == 2
