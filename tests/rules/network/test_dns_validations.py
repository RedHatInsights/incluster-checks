...
class TestDnsReachabilityCollector(DataCollectorTestBase):
    ...
    scenarios = [
        DataCollectorScenarioParams(
            ...
            {
                "dig +short +time=2 +tries=1 @192.168.1.1 google.com": CmdOutput("142.250.185.46\n"),
                "dig +short +time=2 +tries=1 @8.8.8.8 google.com": CmdOutput("142.250.185.46\n"),
            },
            scenario_res={
        ...
        DataCollectorScenarioParams(
            ...
            {
                "dig +short +time=2 +tries=1 @192.168.1.1 google.com": CmdOutput("142.250.185.46\n"),
                "dig +short +time=2 +tries=1 @8.8.8.8 google.com": CmdOutput("", return_code=1),
            },
            scenario_res={
    ...
    def test_collect_data(self, scenario_params, tested_object):
        """Test collect_data with dns_servers parameter."""
        self._init_data_collector_object(tested_object, scenario_params)
        result = tested_object.collect_data(dns_servers=self.dns_servers)
...
class TestVerifyDnsReachability(RuleTestBase):
    ...
    scenario_failed = [
        RuleScenarioParams(
            ...
            failed_msg="DNS servers from cluster upstream resolvers unreachable from all nodes: 192.168.1.1, 8.8.8.8",
        ),
        RuleScenarioParams(
            ...
            failed_msg="No DNS servers found. Neither upstream DNS resolvers configured nor nameservers in /etc/resolv.conf.",
        ),
        RuleScenarioParams(
            ...
            failed_msg="Failed to test DNS reachability from 1 node(s): node-1. Cannot verify DNS reachability with incomplete data.",
        ),
    ]
