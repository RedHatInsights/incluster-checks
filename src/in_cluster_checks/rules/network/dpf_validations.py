...
class DpuBondLacpHealth(BondBase):
    ...
    def run_rule(self) -> RuleResult:
        ...
        for bond_name in bond_names:
            ...
            if missing_agg:
                all_issues.append(
                    f"{bond_name}: missing aggregator_id on slave(s): " f"{', '.join(missing_agg)}, LACP not negotiated"
                )
            else:
                ...
                if len(agg_ids) > 1:
                    ...
                    all_issues.append(
                        f"{bond_name}: slaves in different aggregators " f"({details}), LACP not fully negotiated"
                    )
            ...
            if not down_slaves and not missing_agg and not churned:
                ...
                if len(agg_ids) == 1:
                    ...
                    all_passed.append(f"{bond_name}: LACP healthy, " f"{len(info['slaves'])} slaves UP ({slave_info})")
...
class OvnGeneveTunnelLocalIp(Rule):
    ...
    local_ip in the tunnel options. If the node's IP changes (e.g.
    during
    ...
    def run_rule(self) -> RuleResult:
        ...
        if len(local_ips) > 1:
            return RuleResult.failed(
                f"Multiple different local_ip values in Geneve tunnels: "
                f"{sorted(local_ips)}. OVS configuration may be inconsistent."
            )
